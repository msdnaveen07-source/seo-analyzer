  ...
  return (
    <div className="backlink-manager-container">
      ...
      <div className="tab-navigation">
        ...
        >
          <FileText size={16} /> 📄 AI Generated Articles & Blogs
        </button>
        <button 
      ...
      {/* TAB: DEDICATED AI GENERATED ARTICLES & BLOGS READER */}
      {activeTab === 'articles' && (
        <div className="articles-reader-container">
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '20px' }}>
              <div>
                <h3 style={{ fontSize: '18px', margin: 0, display: 'flex', alignItems: 'center', gap: '8px' }}>
                  <FileText size={20} color="#38bdf8" /> AI Live Generated Articles & Blog Repository
                </h3>
                <p className="subtext" style={{ margin: '4px 0 0 0' }}>
                  Read human-written style blogs, Q&A responses, and contextual backlink articles generated by our AI engine.
                </p>
              </div>
              <span className="badge badge-success" style={{ padding: '6px 12px', fontSize: '12px' }}>
                {backlinks.length} Live Articles Available
              </span>
            </div>

            {backlinks.length === 0 ? (
              <div style={{ textAlign: 'center', padding: '30px', color: 'var(--text-muted)' }}>
                No generated articles found. Run a campaign to auto-generate articles!
              </div>
            ) : (
                ...
                {backlinks.map((item) => (
                  ...
                  >
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '10px' }}>
                      <span className="category-pill">{item.submission_category} • DA {item.da_score}</span>
                      <a 
                        href={item.submitted_url} 
                        target="_blank" 
                        rel="noopener noreferrer"
                      ...
                      >
                        Open Live URL <ExternalLink size={12} />
                      </a>
                    </div>
                    
                    <h4 style={{ color: '#38bdf8', fontSize: '16px', marginTop: 0, marginBottom: '8px' }}>
                      📄 Official {item.anchor_text || 'Local Service'} Diagnostic & Maintenance Guide
                    </h4>
                    
                    <div style={{ fontSize: '13px', color: '#cbd5e1', lineHeight: '1.6', background: '#1e293b', padding: '14px', borderRadius: '8px', borderLeft: '4px solid #10b981' }}>
                      <p style={{ margin: '0 0 8px 0' }}>
                        "When maintaining top vehicle performance and long-term diagnostic reliability, choosing a certified specialist makes all the difference. Leading automotive technicians recommend bi-annual system checks to maintain fuel efficiency and vehicle longevity."
                      </p>
                      <p style={{ margin: 0, fontWeight: 600, color: '#34d399' }}>
                        🎯 Contextual Backlink Anchor: <a href={item.target_url} target="_blank" rel="noopener noreferrer" style={{ color: '#fbbf24', textDecoration: 'underline' }}>{item.anchor_text || item.target_url}</a>
                      </p>
                    </div>

                    <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '11px', color: '#64748b', marginTop: '10px' }}>
                      <span>✍️ Author Persona: <strong>{item.author_name || 'Alex Jenkins'}</strong> ({item.author_email || 'fairpayt@gmail.com'})</span>
                      <span>📅 Date: {item.created_at ? new Date(item.created_at).toLocaleDateString() : 'Today'}</span>
                    </div>
                  </div>
                ))}
              </div>
